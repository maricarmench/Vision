...
Public Class DV_CristalIngresoMasivoView
    ...
    Public Sub New(documento As Tmp_DocumentoSalidaEntity)

        Me.New()

        _entityToEdit = documento.DeepClone()
        _entityToEdit.Detalles.Clear()
        Dim unCajaEntity As CajaEntity = CajaController.BuscarPorId(documento.CajaId)
        If Not String.IsNullOrEmpty(unCajaEntity.XML) Then
            _parametroCaja = ParametroCaja.FromXML(unCajaEntity.XML)
        End If
    End Sub
    ...
    Private _entityToEdit As Tmp_DocumentoSalidaEntity
    Public ReadOnly Property EntityToEdit As Tmp_DocumentoSalidaEntity
        Get
    ...
    Private Function DatosOk() As Boolean
        ...
        End If
        If cboBonificacionId.HasValue AndAlso Not Articulo_LocalController.ArticuloBonificable(_entityToEdit.LocalId, CInt(cboCristalId.EditValue)) Then
            DxErrorProvider1.SetError(cboBonificacionId, String.Format("El artículo no es bonificable. Para poder cargarle una bonificación debe marcarlo como bonificable en el local {0} - {1}", _entityToEdit.LocalId, LocalController.DescripcionFromId(_entityToEdit.LocalId)))
    ...
    Private Sub AgregarRenglon()
        ...
        With toAdd
            ...
            .ImporteUnitario = tmp_DocSalidaDetalleController.CalcularPrecioUnitarioDeArticulo(toAdd)
    ...
    Public Overrides Sub DisposeDataView()
        If cboCristalId.Properties.DataSource IsNot Nothing Then
    ...
    Private Sub AgregarArticulos(cristalId As Integer)
        ...
        Using da As IDataAccessAdapter = Studio.Net.Helper.DAL.DataAccessAdapter.Create(True)
            ...
            Else
                ...
                AgregarADetalle(EntityToEdit.Detalles, cristal, String.Empty, cantidad:=NumberConversor.ParseDecimal(txtCantidad.Text), cilindrico:=NumberConversor.ParseDecimal(txtCilindrico.Text),
    ...
    Private Sub AgregarADetalle(coltoAdd As Studio.Phone.POS.DAL.HelperClasses.EntityCollection(Of Tmp_DocSalidaDetalleEntity), articulo As ArticuloEntity, datoExtra As String, Optional cantidad As Decimal = Decimal.One, Optional cilindrico As Decimal = Decimal.MinValue, Optional esferico As Decimal = Decimal.MinValue, Optional adicion As Decimal = Decimal.MinValue, Optional descripcionOverride As String = Nothing, Optional bonificacionId As Integer = 0)
        ...
        Dim detalle As Tmp_DocSalidaDetalleEntity

        Dim importe As Decimal = CalcularImporteUnitario(articulo, esferico, cilindrico, adicion)
        ...
        If detalle Is Nothing Then
            detalle = New Tmp_DocSalidaDetalleEntity With {.ArticuloId = articulo.Id, .ArticuloCodigo = articulo.Id.ToString(), .Descripcion = descripcion, _
    ...
    Friend Function CalcularImporteUnitario(ByVal articulo As ArticuloEntity, esferico As Decimal, cilindrico As Decimal, adicion As Decimal) As Decimal
        ...
        If cristal IsNot Nothing Then
            If cristal.ArticuloIdGuia = 0 Then
                Return DV_Cristal_PrecioGraduacionBEntity.CalularPrecioUnitarioDeArticulo(articulo.Id, EntityToEdit.ListaPreVtaId, EntityToEdit.MonedaId, esferico, cilindrico, adicion)
            End If
        End If
    ...
    Private Sub lqifsBonificacion_GetQueryable(sender As Object, e As DevExpress.Data.Linq.GetQueryableEventArgs) Handles lqifsBonificacion.GetQueryable
        ...
        e.QueryableSource = (New BonificacionBEntity).GetDataForComboAsQueryable(da, _entityToEdit.LocalId)
        e.Tag = da
    End Sub
