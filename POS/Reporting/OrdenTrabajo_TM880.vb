...
Public Class OrdenTrabajoRecetaComun_TM880
    ...
    Public Sub Proceso(venta As Phone.POS.DAL.EntityClasses.DocumentoSalidaEntity) Implements IImprimirFactura.Proceso
        ...
        Const C_INT_ANCHO As Integer = 55
        ...
        Using da As IDataAccessAdapter = Studio.Net.Helper.DAL.DataAccessAdapter.Create()
            ...
            With receta
                ...
                Dim prn As New Studio.Net.Printing.Format.FormatoTicket.PrinterManager
                Try
                    prn.OpenPrinter(ConfigReaderSpecific.GetDefaultPrinterName(.CajaId, .DocumentoTipoId))
                    prn.NewDocument(False)
                    ...
                    If signo < Decimal.Zero Then
                        prn.Printer.SetAlignment(Net.Printing.EpsonTMU950Printer.Aligment.Center)
                        prn.Printer.SetFontBold(True)
                        prn.Printer.WriteLine(">> DEVOLUCION <<")
                        prn.Printer.NewLine()
                    End If
                    ...
                    prn.ReserPrinter()
                    prn.Printer.WriteLine(cabezal)
                    prn.Printer.NewLine()
                    prn.Printer.SetFontBold(True)
                    prn.Printer.SetAlignment(Net.Printing.EpsonTMU950Printer.Aligment.Center)
                    prn.Printer.Font.DoubleHeight = True
                    prn.Printer.Font.DoubleWidth = True
                    prn.Printer.WriteLine(DocumentoTipoController.DescripcionFromId(da, .DocumentoTipoId))
                    prn.ReserPrinter()
                    prn.Printer.NewLine()
                    'Nro. documento
                    prn.Printer.SetFontBold(True)
                    ...
                    prn.Printer.Write(Chr(&H1B) & Chr(&H21) & Chr(2))
                    prn.Printer.SetFontBold(True)
                    prn.Printer.WriteLine(linea)
                    prn.Printer.Write(Chr(&H1B) & Chr(&H21) & Chr(1))
                    prn.Printer.SetFontBold(False)
                    prn.Printer.Font.DoubleWidth = False
                    'Cliente
                    linea = "Cliente: " & .ClienteId.ToString() & " - " & .Nombre
                    prn.Printer.WriteLine(linea)
                    'Fecha emision
                    linea = "Fecha emision: " & .FechaEmitida.ToString("d") & "    " & "Fecha entrega: " & .FechaEntrega.ToString("d")
                    prn.Printer.WriteLine(linea)

                    Dim separador As String = StrDup(C_INT_ANCHO, "-")
                    prn.Printer.WriteLine(separador)
                    ...
                    If .CristalIdLejosDerecho > 0 OrElse .CristalIdOjoLejosIzquierdo > 0 OrElse .ArmazonIdLejos > 0 OrElse .ServiciosLejos.Count > 0 Then
                        ...
                        prn.Printer.Write(Chr(&H1B) & Chr(&H21) & Chr(2))
                        prn.Printer.SetFontBold(True)
                        prn.Printer.WriteLine("LENTE DE LEJOS:")
                        ...
                        If .CristalIdLejosDerecho > 0 Then
                            prn.Printer.Font.DoubleWidth = True
                            'prn.Printer.SetFontBold(True)
                            prn.Printer.WriteLine(String.Format("Derecho"))
                            prn.Printer.Font.DoubleWidth = False
                            prn.Printer.SetFontBold(False)
                            prn.Printer.WriteLine(separador)
                            ...
                            prn.Printer.Write(Chr(&H1B) & Chr(&H21) & Chr(2))
                            prn.Printer.SetFontBold(True)
                            prn.Printer.WriteLine("    Eje" & "    Cil" & "    Esf" & "     Ad" & "    Dst" & "    Alt")
                            prn.Printer.SetFontBold(False)
                            'Eje
                            ...
                            If .CilindricoOjoLejosDerecho <> .CilindricoOjoLejosDerechoCalc Then
                                ...
                                prn.Printer.Write(Chr(&H1B) & Chr(&H21) & Chr(2))
                                prn.Printer.SetFontBold(True)
                                prn.Printer.WriteLine("(TRANSP) Eje" & "    Cil" & "    Esf")
                                prn.Printer.SetFontBold(False)
                                'Eje
                            ...
                            prn.Printer.Font.DoubleWidth = False
                            prn.Printer.SetFontBold(True)
                            prn.Printer.Write("Material:" & Espacio06)
                            prn.Printer.SetFontBold(False)
                            ...
                            prn.Printer.SetFontBold(True)
                            'Cristal
                            prn.Printer.Write("Cristal:" & Espacio07)
                            prn.Printer.SetFontBold(False)
                            ...
                            prn.Printer.WriteLine(linea)
                            prn.Printer.SetFontBold(True)
                            prn.Printer.WriteLine(String.Format("Unidad(es):{0}", .CantidadOjoLejosDerecho))
                            prn.Printer.SetFontBold(False)
                            prn.Printer.WriteLine(separador)
                            prn.Printer.SetFontBold(True)
                        End If
                        ...
                        If .CristalIdLejosIzquierdo > 0 Then
                            prn.Printer.SetFontBold(False)
                            prn.Printer.Font.DoubleWidth = True
                            prn.Printer.WriteLine(String.Format("Izquierdo"))
                            prn.Printer.Font.DoubleWidth = False
                            'prn.Printer.SetFontBold(False)
                            prn.Printer.WriteLine(separador)
                            'Eje, Cil, Esf, Ad, Dst, Alt
                            ...
                            prn.Printer.WriteLine(linea)
                            prn.Printer.SetFontBold(True)
                            prn.Printer.WriteLine(String.Format("Unidad(es):{0}", .CantidadOjoLejosIzquierdo))
                            prn.Printer.SetFontBold(False)
                            prn.Printer.WriteLine(separador)
                            prn.Printer.SetFontBold(True)
                        End If
                        'Armazon
                        prn.Printer.SetFontBold(True)
                        prn.Printer.Write("Armazon:" & Espacio07)
                        prn.Printer.SetFontBold(False)
                        ...
                        prn.Printer.WriteLine(linea)
                        prn.Printer.SetFontBold(True)
                        prn.Printer.WriteLine(linea1)
                        'prn.Printer.SetFontBold(True)
                        'Servicios
                        prn.Printer.WriteLine("Servicios:")
                        prn.Printer.SetFontBold(False)
                        For Each servicio As DocSalidaDetalleEntity In .ServiciosLejos
                            prn.Printer.WriteLine(Espacio15 & servicio.Descripcion)
                        Next
                    End If
                    'LENTE DE CERCA
                    If .CristalIdCercaDerecho > 0 OrElse .CristalIdOjoCercaIzquierdo > 0 OrElse .ArmazonIdCerca > 0 OrElse .ServiciosCerca.Count > 0 Then
                        ...
                        prn.Printer.Write(Chr(&H1B) & Chr(&H21) & Chr(2))
                        prn.Printer.SetFontBold(True)
                        prn.Printer.WriteLine("LENTE DE CERCA:")
                        prn.Printer.SetFontBold(False)
                        prn.Printer.Write(Chr(&H1B) & Chr(&H21) & Chr(1))
                        prn.Printer.WriteLine(separador)
                        prn.Printer.SetFontBold(True)
                        ...
                        If .CristalIdCercaDerecho > 0 Then
                            ...
                            prn.Printer.Write(Chr(&H1B) & Chr(&H21) & Chr(2))
                            prn.Printer.SetFontBold(True)
                            prn.Printer.WriteLine("    Eje" & "    Cil" & "    Esf" & "    Dst" & "    Alt")
                            prn.Printer.SetFontBold(False)
                            'Eje
                            ...
                            prn.Printer.WriteLine(String.Format("Unidad(es):{0}", .CantidadOjoCercaDerecho))
                            prn.Printer.SetFontBold(False)
                            prn.Printer.WriteLine(separador)
                            prn.Printer.SetFontBold(True)
                        End If
                        ...
                        If .CristalIdCercaIzquierdo > 0 Then
                            ...
                            prn.Printer.SetFontBold(True)
                            prn.Printer.WriteLine(String.Format("Unidad(es):{0}", .CantidadOjoCercaIzquierdo))
                            prn.Printer.SetFontBold(False)
                            prn.Printer.WriteLine(separador)
                            prn.Printer.SetFontBold(True)
                        End If
                    ...
                    prn.Printer.SetFontBold(False)
                    prn.Printer.Font.DoubleWidth = False
                    prn.Printer.Font.DoubleHeight = False


                    prn.EndDocument(True)
