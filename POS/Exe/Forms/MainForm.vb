    ...
    Protected Overrides Function OnLogin() As Boolean
        ...
        Try
            ...
            If ConfigReaderSpecific.EjecutarTraffic() Then
                IniciarTraffic()
            End If
            If ConfigReaderSpecific.EjecutarFE() AndAlso Studio.Phone.POS.BLL.CajaController.AdheridaFacturaElectronica(Nothing, ConfigReaderSpecific.GetCajaId()) Then
                IniciarFE()
            End If
...
#Region "Traffic"
    ...
    Private _feActualizer As New Studio.Phone.Traffic.BLL.FEActualizer
    ...
    Private m_manager As Studio.Net.Helper.Threading.InvokerManager
    Private m_lckDummy As New Object
    ...
    Private Sub IniciarTraffic()
        ...
        m_manager = New Studio.Net.Helper.Threading.InvokerManager
        _managerThread = New System.Threading.Thread(AddressOf m_manager.StarCheck)
        m_manager.AddInvokeMethod(New simpleDel(AddressOf _actualizer.GenerarTareasPendiente))
        m_manager.AddInvokeMethod(New simpleDel(AddressOf _actualizer.ActualizarDBEnPOSDesdeTareasPendiente))
        m_manager.AddInvokeMethod(New simpleDel(AddressOf _actualizer.TransferirDatosATraffic))
        m_manager.AddInvokeMethod(New simpleDel(AddressOf _actualizer.CheckFechaYHora))
    ...
    Private Sub FinalizarTraffic()
        ...
        If m_manager IsNot Nothing Then

            m_manager.Abort = True
            _managerThread.Abort()

            If _actualizer IsNot Nothing Then
    ...
    <SecurityPermissionAttribute(SecurityAction.Demand, ControlThread:=True)>
    Private Sub FinalizarFE()

        If m_manager IsNot Nothing Then

            m_manager.Abort = True
            If _managerThread IsNot Nothing Then
                _managerThread.Abort()
            End If

            _feActualizer = Nothing
            _managerThread = Nothing
    ...
    Private Sub IniciarFE()

        _feActualizer.CajaId = ConfigReaderSpecific.GetCajaId()

        '_feActualizerThread = New System.Threading.Thread(AddressOf Me._feActualizer.RevisarTransaccionesPendientes)
        ...
        If m_manager Is Nothing Then
            ...
            _managerThread = New System.Threading.Thread(AddressOf m_manager.StarCheck)
            _managerThread.IsBackground = True
            _managerThread.Priority = Threading.ThreadPriority.Lowest
            _managerThread.Start()
        End If

        SyncLock m_manager
            m_manager.AddInvokeMethod(New simpleDel(AddressOf _feActualizer.RevisarTransaccionesPendientes))
        End SyncLock
        'End SyncLock



        '_manager.AddThreadForCheck(_feActualizerThread)



        '_trafficThread.Start()
        '_actualizerThread.Start()
        '_senderThread.Start()
    ...
    Private Sub CambiarPanelRecepcion(ByVal toolTipText As String, ByVal isError As Boolean)
        With bsiDownloading
            ...
            Else
                If isError Then
                    .Glyph = Global.Studio.Vision.POS.Exe.My.Resources.Download___Error
                Else
                    .Glyph = Global.Studio.Vision.POS.Exe.My.Resources.Download
                End If
            End If
        End With
    End Sub
    ...
    Private Sub CambiarPanelEnvio(ByVal toolTipText As String, ByVal isError As Boolean)
        With bsiUploading
            ...
            Else
                If isError Then
                    .Glyph = Global.Studio.Vision.POS.Exe.My.Resources.Upload___Error
                Else
                    .Glyph = Global.Studio.Vision.POS.Exe.My.Resources.Upload
                End If
            End If
        End With
    End Sub
    ...
    Private Sub CambiarPanelActualizacion(ByVal toolTipText As String, isError As Boolean)
        ...
        With bsiWriting
            ...
            Else
                If isError Then
                    .Glyph = Global.Studio.Vision.POS.Exe.My.Resources.Write___Error
                Else
                    .Glyph = Global.Studio.Vision.POS.Exe.My.Resources.Write
                End If
            End If
        End With
